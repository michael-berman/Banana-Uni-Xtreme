...
const Coords = __webpack_require__(6);
...
class Level1Obstacle{
  constructor(){
    ...
    this.spritePos = [null, null, null];
    this.spriteFrame = 0;
    this.fruitBowlPos = 14500;
    this.skyFrame = 0;
    this.skyMonkeyPos = SkyMonkeyPos.generateSkyPos();
    this.skyMonkeyPosX = [null, null, null];
    this.endGame = false;
    this.backgroundSpritePos = 0;
  ...
  draw(ctx, frames){
    this.drawBackground(ctx, frames);
  ...
  drawBackground(ctx, frames){
    ...
    background.src = './assets/sprite_background.jpg';
    this.backgroundSpritePos += this.offset * 0.2;
    let posX = (this.backgroundSpritePos) % 3600;
    console.log(posX)
    ctx.drawImage(background, posX, 0, 500, 400, 0, 0, 700, 500);
  }
